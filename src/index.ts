...
const GEMINI_MCP_PATH =
  process.env.GEMINI_MCP_PATH ||
...
let modelReasoningMap: Map<string, ReasoningEffort[]> = new Map();
let modelReasoningLoaded = false;

async function loadModelReasoningSupport(client: CopilotClient): Promise<void> {
  if (modelReasoningLoaded) return;
  try {
    const models: ModelInfo[] = await client.listModels();
    for (const m of models) {
      if (m.supportedReasoningEfforts && m.supportedReasoningEfforts.length > 0) {
        modelReasoningMap.set(m.id, m.supportedReasoningEfforts as ReasoningEffort[]);
      }
    }
...
function getModelReasoningLevels(model: string): ReasoningEffort[] | undefined {
...
const DEFAULT_REASONING_EFFORT = parseReasoningEffort(
...
function buildSystemMessage(geminiModel: string): string {
...
let singletonClient: CopilotClient | undefined;
let clientInitPromise: Promise<CopilotClient> | undefined;
...
async function runBridge(options: BridgeOptions): Promise<string> {
  const {
    prompt,
    model = DEFAULT_MODEL,
    ...
    writeAccess = DEFAULT_WRITE_ACCESS,
  } = options;
  ...
  log(
    "INFO",
    `Starting bridge: model=${model}, reasoning=${effectiveEffort ?? "n/a"}, gemini=${geminiModel}, write=${writeAccess}, timeout=${timeout}ms`
  );
  ...
  const geminiMcpConfig: MCPLocalServerConfig = {
    type: "local",
    command: "node",
  ...
  try {
    ...
    log(
      "INFO",
      `Response received. Tools used: ${events.join(", ") || "none"}`
    );
          ...
          inputSchema: {
            ...
            properties: {
              prompt: {
                type: "string",
                description:
                  "The prompt/question to send to GPT-5.3 Codex. Be specific about what you want analyzed.",
              },
              model: {
                type: "string",
...
server.setRequestHandler(
  ...
  async (request: CallToolRequest): Promise<CallToolResult> => {
    ...
    const validatedGeminiModel =
      geminiModel && VALID_GEMINI_MODELS.includes(geminiModel)
        ? geminiModel
        : undefined;
...
function validateConfig() {
  if (!existsSync(GEMINI_MCP_PATH)) {
    log(
      "WARN",
      `Gemini MCP not found at ${GEMINI_MCP_PATH} - tool calls will fail until it's installed`
    );
  }

  const envEffort = process.env.COPILOT_REASONING_EFFORT;
  if (envEffort && !VALID_REASONING_EFFORTS.includes(envEffort as ReasoningEffort)) {
    log(
      "WARN",
      `Invalid COPILOT_REASONING_EFFORT="${envEffort}", using "high". Valid: ${VALID_REASONING_EFFORTS.join(", ")}`
    );
  }
  ...
  if (envGeminiModel && !VALID_GEMINI_MODELS.includes(envGeminiModel)) {
    log(
      "WARN",
      `Unrecognized GEMINI_MODEL="${envGeminiModel}" - passing through (may be a new model)`
    );
  }
}
...
async function main() {
  ...
  log(
    "INFO",
    `Config: model=${DEFAULT_MODEL}, reasoning=${DEFAULT_REASONING_EFFORT}, write=${DEFAULT_WRITE_ACCESS}, timeout=${DEFAULT_TIMEOUT}ms`
  );
